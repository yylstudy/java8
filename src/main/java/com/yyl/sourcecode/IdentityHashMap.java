        ...
        implements Map<K,V>, java.io.Serializable, Cloneable
{
     ...
     */
    //hash函数用于散列，并且保证元素的散列值在数组的由此索引，值在key的偶数索引+1的索引
    ...
    public V get(Object key) {
        ...
        Object[] tab = table;
        int len = tab.length;
        int i = hash(k, len);//key的hash计算
        while (true) {
            Object item = tab[i];
            if (item == k)//判断地址相等 即完全相等的两个对象
                return (V) tab[i + 1];//key的索引加1 就是值的索引
            if (item == null)
                return null;
            i = nextKeyIndex(i, len); //取下一个key的索引，要跨过value的索引，往后移两单位;
        }
    }
    ...
    public V put(K key, V value) {
        ...
        retryAfterResize: for (;;) {
            ...
            for (Object item; (item = tab[i]) != null;
                 i = nextKeyIndex(i, len)) {
                if (item == k) {//经过hash计算key值相等
                    @SuppressWarnings("unchecked")
                    V oldValue = (V) tab[i + 1];//
                    tab[i + 1] = value;//新的值赋予旧的值
                    return oldValue;
                }
            }
