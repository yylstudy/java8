        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
     ...
     */
    private static final Object[] EMPTY_ELEMENTDATA = {};//一个空数组 当用户指定该 ArrayList 容量为 0 时，返回该空数组
    ...
    /**
     *  一个空数组实例 当用户没有指定 ArrayList 的容量时(即调用无参构造函数)，
     ...
     * 该值为 DEFAULTCAPACITY_EMPTY_ELEMENTDATA 时，当第一次添加元素进入 ArrayList
     * 中时，数组将扩容值 DEFAULT_CAPACITY(10)
     */
    ...
    public void trimToSize() {
        ...
        if (size < elementData.length) {
            ...
            elementData = (size == 0)
                    ? EMPTY_ELEMENTDATA
                    : Arrays.copyOf(elementData, size);//这个方法就是将数组长度变成size的长度，节省资源
        }
    }
    ...
    private void grow(int minCapacity) {
        ...
        if (newCapacity - minCapacity < 0)//若 newCapacity 依旧小于 minCapacity  直接取minCapacity的值为newCapacity的值
            newCapacity = minCapacity;
        if (newCapacity - MAX_ARRAY_SIZE > 0)
            newCapacity = hugeCapacity(minCapacity);//若 newCapacity 大于最大存储容量，则进行大容量分配
     ...
     * 多线程环境下迭代ArrayList时可能会报ConcurrentModificationException异常，因为
